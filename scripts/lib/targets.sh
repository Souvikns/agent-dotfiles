...
#
# Top-level skills/ is one source linked into BOTH tools. OpenCode also reads
# ~/.claude/skills natively (verified 2026-09-09), so the opencode link is
# strictly redundant today — it is kept deliberately so the setup survives
# either tool changing that fallback. Linking one source twice produces no
# duplicates: OpenCode dedupes by skill name.
#
...
# ~/.claude.json were all verified NOT to be read. ~/.claude/CLAUDE.md is read
# only as a FALLBACK when ~/.config/opencode/AGENTS.md does not exist - so the
# AGENTS.md this repository links suppresses it. That is intended: shared
